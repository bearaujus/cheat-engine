...
`make build` always writes the Release x64 executable to `Cheat Engine\bin\cheatengine-x86_64.exe`. `make run` starts that executable immediately when its exact build-input fingerprint is unchanged and performs a clean rebuild only when it is missing or stale. `make smoke` always builds it, rejects unexpected startup dialogs, confirms the normal main window appears, and closes only the instance it launched. The IDE-only `Debug 64-Bit` mode writes `Cheat Engine\bin\cheatengine-x86_64-debug.exe`. Local runs and smoke tests pass `NOAUTORUN` and `NOFIRSTTIME`, so they do not load autorun extensions or show first-run language/tutorial prompts; normal launches without those arguments are unchanged.
